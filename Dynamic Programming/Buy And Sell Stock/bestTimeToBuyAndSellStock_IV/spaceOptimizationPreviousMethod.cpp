/**
 * time and space complexity. The time complexity of the algorithm is O(n*k), where n is the number of prices
 * and k is the maximum number of transactions. The space complexity is O(k).
 * K transactions are allowed where buy and sell are considered as one transaction.
 * This code solves the problem of finding the maximum profit that can be obtained by buying and selling stocks,
 * given a maximum number of transactions allowed. It uses tabulation with space optimization to optimize the
 */
